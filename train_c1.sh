...
#SBATCH --job-name=anlp_c1
#SBATCH --output=c1_log.txt
#SBATCH --error=c1_log.txt
...
echo "[$(date)] Job ${SLURM_JOB_ID:-local}: starting C1 training"
...
echo "[$(date)] Environment ready; launching C1 final train/validation/test run"
echo "Python interpreter: $CONDA_PREFIX/bin/python3"
"$CONDA_PREFIX/bin/python3" -u src/train.py C1 \
...
echo "[$(date)] C1 training completed successfully"
