...
#SBATCH --output=c4_log.txt
#SBATCH --error=c4_log.txt
...
echo "[$(date)] Job ${SLURM_JOB_ID:-local}: starting C4 training"
...
echo "[$(date)] Environment ready; launching C4 final train/validation/test run"
echo "Python interpreter: $CONDA_PREFIX/bin/python3"
"$CONDA_PREFIX/bin/python3" -u src/train.py C4 \
...
echo "[$(date)] C4 training completed successfully"
