...
#SBATCH --job-name=anlp_c2
#SBATCH --output=c2_log.txt
#SBATCH --error=c2_log.txt
...
echo "[$(date)] Job ${SLURM_JOB_ID:-local}: starting C2 training"
...
echo "[$(date)] Environment ready; launching C2 final train/validation/test run"
echo "Python interpreter: $CONDA_PREFIX/bin/python3"
"$CONDA_PREFIX/bin/python3" -u src/train.py C2 \
...
echo "[$(date)] C2 training completed successfully"
